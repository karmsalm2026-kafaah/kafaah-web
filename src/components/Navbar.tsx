...
const getServiceIcon = (slug: string) => {
  switch (slug) {
    case "owners-engineer": return <ShieldCheck className="w-4 h-4" />;
    case "commissioning": return <Power className="w-4 h-4" />;
    case "operation-readiness": return <ClipboardCheck className="w-4 h-4" />;
    case "troubleshooting": return <Search className="w-4 h-4" />;
    case "production-optimization": return <TrendingUp className="w-4 h-4" />;
    case "operator-training": return <GraduationCap className="w-4 h-4" />;
    case "investor-advisory": return <Briefcase className="w-4 h-4" />;
                ...
                <div className="p-1.5">
                  {LOCALES.map((l) => (
                    <button
                      ...
                      className={`w-full text-start px-4 py-2.5 rounded-sm text-[13px] transition-all duration-200 ${locale === l
